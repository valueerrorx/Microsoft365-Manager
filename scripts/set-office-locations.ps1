...
if (-not (Test-Path -LiteralPath $MappingsPath)) {
    Write-JsonResult @{
        status      = "error"
        message     = "Mappings-Datei nicht gefunden: $MappingsPath"
        updated     = 0
        failed      = 0
        updatedUpns = @()
        errors      = @()
...
try {
  ...
  $parsed = $raw | ConvertFrom-Json
    ...
    Write-JsonResult @{
        status      = "error"
        message     = "Mappings JSON ungueltig: $($_.Exception.Message)"
        updated     = 0
        failed      = 0
        updatedUpns = @()
        errors      = @()
...
if ($mappingList.Count -eq 0) {
    Write-JsonResult @{
        status      = "error"
        message     = "Keine gueltigen Mappings"
        updated     = 0
        failed      = 0
        updatedUpns = @()
        errors      = @()
...
for ($offset = 0; $offset -lt $mappingList.Count; $offset += $batchSize) {
    ...
    try {
        ...
        foreach ($item in $chunk) {
            $errorItems.Add(@{ upn = $item.upn; message = $batchErr })
            Write-Host "FEHLER ($($item.upn)): $batchErr"
        }
    }
}
